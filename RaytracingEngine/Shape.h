...
public:
	...
	virtual std::optional<Vec3> getNormalAt(const Vec3& point) const { return std::nullopt; }
...
public:
	...
	std::optional<Vec3> getNormalAt(const Vec3& point) const override {
		Vec3 normal = (point - getPosition()).normalize();
		return normal;
	}
